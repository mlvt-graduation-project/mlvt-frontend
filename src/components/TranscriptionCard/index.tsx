...
interface TranscriptionCardProps {
    ...
    isTranslationTarget?: boolean; // To show the language switcher
    sourceLanguage?: string; // e.g. "Auto Detect"
...
const TranscriptionCard: React.FC<TranscriptionCardProps> = ({
    ...
    const cardBgColor = 'rgba(70, 70, 130, 0.3)'; // More purple, less opaque
    ...
    return (
        <Stack spacing={2} sx={{position: 'relative'}}>
            ...
            >
                ...
                <Box flexGrow={1} /> {/* Pushes volume icon to the right */}
                <IconButton size="small" sx={{ color: 'grey.400' }}>
            ...
            {isTranslationTarget && sourceLanguage && targetLanguage && (
                ...
                >
                <Button variant="text" size="small" sx={{ color: 'grey.300', borderRadius: 50, px:2 }}>
                    {sourceLanguage}
                </Button>
                <IconButton size="small" sx={{ color: 'primary.main', bgcolor: 'rgba(255,255,255,0.1)', '&:hover': {bgcolor: 'rgba(255,255,255,0.2)'} }}>
                    <RefreshIcon fontSize="small" />
                </IconButton>
                <Button variant="contained" size="small" color="primary" sx={{ borderRadius: 50, px:2, boxShadow: 'none', textTransform: 'none' }}>
                    {targetLanguage}
                </Button>
                </Paper>
            )}

            {!isTranslationTarget && sourceLanguage && ( // For the first card's language button
                <Paper
                    elevation={1}
                    sx={{
                        p: 1,
                        bgcolor: 'rgba(0,0,0,0.2)',
                        borderRadius: 50, // Pill shape
                        display: 'inline-flex', // To make it wrap content
                        alignItems: 'center',
                ...
                >
                    <Button variant="text" size="small" sx={{ color: 'grey.300', borderRadius: 50, px:2, textTransform: 'none'  }}>
                        {sourceLanguage}
                    </Button>
                </Paper>
            )}
