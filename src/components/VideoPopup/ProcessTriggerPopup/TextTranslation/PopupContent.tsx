...
export const DialogContent: React.FC = () => {
    ...
    const handleGenerateFileFromDevice = useCallback(async (): Promise<number> => {
        if (deviceFile && sourceLanguage) {
            try {
                handleChangeTextData({ lang: getLanguageCode(sourceLanguage) });
                const textId = await uploadText(deviceFile, textData, TextFileType.PlainText);
                return textId;
            } catch (error) {
                throw error;
            }
    ...
    }, [sourceLanguage, targetLanguage, inputText, viewState, deviceFile, MLVTText]);
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            try {
                ...
                } else throw new Error('TextId or textLanguage or targetLanguage is null');
            } catch (error) {
